...
// Specialization of FastAbs() kernels for Arm Neon platforms.
...
namespace radio_core::kernel::fast_abs_internal {

template <class T, bool SpecializationMarker>
struct Kernel;

// Specialization for FastAbs<Complex, float>
template <>
  ...
  static inline auto Execute(const std::span<const Complex>& samples,
    ...
    if constexpr (Complex4::kIsVectorized) {
      ...
      while (samples_ptr < aligned_samples_end) {
        ...
        const Float4 abs4 = FastAbs(samples4);
...
// Specialization for FastAbs<HalfComplex, float>
template <>
  ...
  static inline auto Execute(const std::span<const HalfComplex>& samples,
    ...
    // Handle 8 elements at a time.
    {
      ...
      while (samples_ptr < aligned_samples_end) {
        ...
        const Half8 abs4 = FastAbs(samples4);
