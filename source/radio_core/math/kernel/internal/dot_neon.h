...
// Specialization of dot-product kernels for Arm Neon platforms.
  ...
  static inline auto Execute(const std::span<const float>& f,
    ...
    // 30% more speedup compared to 2 registers.
    {
      ...
      while (f_ptr < aligned_f_end) {
        ...
        const Float4 g4_1(g_ptr);
        const Float4 g4_2(g_ptr + 4);
        const Float4 g4_3(g_ptr + 8);
        const Float4 g4_4(g_ptr + 12);
  ...
  static inline auto Execute(const std::span<const Half>& f,
    ...
    // 40% more speedup compared to 2 registers.
    {
      ...
      while (f_ptr < aligned_f_end) {
        ...
        const Half8 g8_1(g_ptr);
        const Half8 g8_2(g_ptr + 8);
        const Half8 g8_3(g_ptr + 16);
        const Half8 g8_4(g_ptr + 24);
