...
// Implementation of the dot product with implicit flip kernel which uses the
// available vectorized types on the current platform. It does not perform any
// more specific optimizations like utilization of multiple registers.
...
namespace radio_core::kernel::experimental::dot_flip_kernel_internal {
...
struct Kernel {
  ...
  static inline auto Execute(const std::span<const FType>& f,
    ...
    const GType* g_ptr = g.data() + num_samples - 1;
    ...
    if constexpr (FType8::kIsVectorized && GType8::kIsVectorized) {
      ...
      while (f_ptr < aligned_f_end) {
        ...
        const GType8 g8 = Reverse(GType8(g_ptr - 7));
    ...
    if constexpr (FType4::kIsVectorized && GType4::kIsVectorized) {
      ...
      while (f_ptr < aligned_f_end) {
        ...
        const GType4 g4 = Reverse(GType4(g_ptr - 3));
