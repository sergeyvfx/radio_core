...
class HorizontalSumBenchmark : public Benchmark {
 public:
  using Benchmark::Benchmark;

 protected:
  auto GetBenchmarkName() -> std::string override {
    return "HorizontalSum<T>()";
  }
  ...
  void Iteration() override {
    ...
    switch (input_sample_type_) {
      case InputSampleType::kFloat: {
        const float sum = kernel::HorizontalSum<float>(float_data_.samples);
        is_finite = IsFinite(sum);
        break;
      }

#if RADIO_CORE_HAVE_HALF
      case InputSampleType::kHalf: {
        const Half sum = kernel::HorizontalSum<Half>(half_data_.samples);
        is_finite = IsFinite(sum);
        break;
      }
#endif
...
auto main(int argc, char** argv) -> int {
  radio_core::benchmark::HorizontalSumBenchmark app;
